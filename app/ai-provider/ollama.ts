...
function getPrompt(rawDiff: string, language: string) {
    return `Create a pull request description in ${language} based on the provided raw Git diff. The description should be in **Markdown format** but **must not be wrapped in code blocks** (e.g., \`\`\`markdown, \`\`\`).  
