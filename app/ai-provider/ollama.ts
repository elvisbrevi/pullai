...
async function formatContentWithAI(rawDiff: string, language: string) {
  console.log(
    `🤖 Formatting content with Ollama in ${
...
The structure of the markdown document should include only the following sections in the mentioned order and language, not more:  
...
- Do **not** add an additional summary at the end or at begin of the document.  
